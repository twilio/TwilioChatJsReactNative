  ...
  describe('Same client', function() {
      ...
      for (let i = 0; i < messagesToSend; i++) {
        promises.push(channel.sendMessage('message' + i).then(messageIndex => {
          ...
          if (index !== -1) {
            ...
            throw new Error('Got unexpected message index ' + messageIndex + ' when sending message (duplicate or out of range)');
          }
        }));
      }
    ...
    it('Verify message indexes left to receive', async () => {
      if (receivedMessageIndexes.length > 0) {
        throw new Error('Expected received messages indexes array to be empty. Indexes left: ' + receivedMessageIndexes.join(' '));
      }
    });

    it('Verify message indexes left to send', async () => {
      if (sentMessageIndexes.length > 0) {
        throw new Error('Expected sent messages indexes array to be empty. Indexes left: ' + sentMessageIndexes.join(' '));
      }
    });
  });
